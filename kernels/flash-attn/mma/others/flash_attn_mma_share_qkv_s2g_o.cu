...
flash_attn_mma_stages_split_q_shared_qkv_s2g_o_kernel(half* Q, 
                                                      half* K, 
                                                      half* V, 
                                                      half* O, 
                                                      int QKV_seqlen,
  ...
  for (int i = 0; i < kWarpTileSeqLenP; ++i) { // 1
    ...
    for (int j = 0; j < kWarpTileHeadDimV; ++j) { // 8
      ...
      int store_lane_smem_O_d  = store_warp_regs_O_d  + (lane_id % 4) * 2; // 0,2,4,6
      int store_smem_O_addr_0  = (store_lane_smem_O_Br + 0) * (kHeadDim + kPadQ) + store_lane_smem_O_d;
      int store_smem_O_addr_1  = (store_lane_smem_O_Br + 8) * (kHeadDim + kPadQ) + store_lane_smem_O_d;
      ...
      //   int store_warp_regs_O_d  = warp_KV * (kMmaAtomN * kWarpTileHeadDimV) + j * kMmaAtomN;
      //   int store_lane_smem_O_Br = store_warp_regs_O_Br + (lane_id / 4); // 0~7
      //   int store_lane_smem_O_d  = store_warp_regs_O_d;
      //   int store_smem_O_addr_0  = (store_lane_smem_O_Br + 0) * (kHeadDim + kPadQ) + store_lane_smem_O_d;
      //   int store_smem_O_addr_1  = (store_lane_smem_O_Br + 8) * (kHeadDim + kPadQ) + store_lane_smem_O_d;
      //   LDST128BITS(Q_tile_smem[store_smem_O_addr_0]) = LDST128BITS(R_Z[0][0]);
      //   LDST128BITS(Q_tile_smem[store_smem_O_addr_1]) = LDST128BITS(R_Z[1][0]);
      // }
    } // end for kWarpTileHeadDimV
  } // end for kWarpTileSeqLenQ
  __syncthreads();
  ...
  int store_gmem_O_addr = ((O_gmem_offset) + 
                           (O_tile_id * Br + load_smem_Q_Br) * kHeadDim + 
                           (load_smem_Q_d));
  #pragma unroll
  for (int i = 0; i < (kHeadDim / (kNumThreads / Br)); i += 8) {
    LDST128BITS(O[store_gmem_O_addr + i]) = LDST128BITS(
      Q_tile_smem[store_smem_O_addr + i]);
  }
}

template<const int kHeadDim, const int kStage>
void launch_flash_attn_mma_stages_split_q_shared_qkv_s2g_o(
  ...
  cudaFuncSetAttribute(
    flash_attn_mma_stages_split_q_shared_qkv_s2g_o_kernel<
      kHeadDim, 
      kMmaAtomM, 
      kMmaAtomN, 
      kMmaAtomK, 
...
void flash_attn_mma_stages_split_q_shared_qkv_s2g_o(torch::Tensor Q, 
  ...
  if (stages > 1) {
    ...
    {
    case 32:
      launch_flash_attn_mma_stages_split_q_shared_qkv_s2g_o<32,  2>(Q, K, V, O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_shared_qkv_s2g_o<64,  2>(Q, K, V, O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_shared_qkv_s2g_o<96,  2>(Q, K, V, O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_shared_qkv_s2g_o<128, 2>(Q, K, V, O);
      break;
    default:
    ...
    {
    case 32:
      launch_flash_attn_mma_stages_split_q_shared_qkv_s2g_o<32,  1>(Q, K, V, O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_shared_qkv_s2g_o<64,  1>(Q, K, V, O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_shared_qkv_s2g_o<96,  1>(Q, K, V, O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_shared_qkv_s2g_o<128, 1>(Q, K, V, O);
      break;
    case 256:
      launch_flash_attn_mma_stages_split_q_shared_qkv_s2g_o<256, 1>(Q, K, V, O);
      break;
    default:
      throw std::runtime_error("headdim not support!");
      break;
