  ...
  for (int k = 0; k < (K_STAGE - 1); ++k) { // 0, 1
    ...
    uint32_t load_smem_b_ptr = (
      smem_b_base_ptr + (k * s_b_stage_offset + 
                         ...
                         swizzle_permuted_B_j<MMA_K>(
                          load_smem_b_n, load_smem_b_k)) * sizeof(half)
    );
    ...
    for (int j = 0; j < WARP_TILE_N; ++j) {
      ...
      uint32_t lane_smem_b_ptr = (
        smem_b_base_ptr + (smem_sel * s_b_stage_offset + 
                           ...
                           swizzle_permuted_B_j<MMA_K>(
                            lane_smem_b_n, lane_smem_b_k)) * sizeof(half)
      );
...
#define LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_TN_X2_KERNEL(stages, stride)\
{                                                                                   \
...
void lanunch_hgemm_mma_m16n8k16_swizzle_tn(
  half* a, half* b, half* c, int M, int N, int K) {
...
int main(int argc, char *argv[]) {
  ...
  printf("ALGO = MMA16816 HGEMM TN MMA=2x4 WARP=4x4 STAGES=2 BLOCK SWIZZLE=2048, SMEM SWIZZLE, B LDMATRIX.X2\n");
#ifndef HGEMM_MMA_DEBUG  
  for (int j = 0; j < 5; j++) {
    int M = M_list[j], N = N_list[j], K = K_list[j];
    float max_error = gemm_error_check_tn<half>(
      lanunch_hgemm_mma_m16n8k16_swizzle_tn<2, 2048>, 
      M, N, K);
...
#define LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4_DSMEM_TN_X2_KERNEL(stages)\
{                                                                             \
  const int smem_max_size = (                                                 \
    (stages) * BM * (BK + A_PAD) * sizeof(half) +                             \
    (stages) * BN * (BK + B_PAD) * sizeof(half));                             \
...
void hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_tn_swizzle(
  ...
  if (swizzle) {
    ...
    {
    case 2: 
      LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_TN_X2_KERNEL(2, swizzle_stride);
      break;
    case 3: 
      LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_TN_X2_KERNEL(3, swizzle_stride);
      break;
    case 4: 
      LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_TN_X2_KERNEL(4, swizzle_stride);
      break;
    case 5: 
      LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_TN_X2_KERNEL(5, swizzle_stride);
      break;
    default:
      LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_TN_X2_KERNEL(2, swizzle_stride);
      break;
