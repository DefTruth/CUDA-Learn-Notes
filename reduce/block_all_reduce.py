...
def run_benchmark(perf_func: callable, values: torch.Tensor, tag: str, 
    ...
    out_info = f"out_{tag}"
    out_val = out.item()
    print(f"{out_info:>17}: {out_val:.8f}, time:{mean_time:.8f}ms")
...
print("-" * 80)
values_f8 = values.to(dtype=torch.float8_e4m3fn)
run_benchmark(lib.block_all_reduce_sum_fp8_e4m3_acc_with_f16, values_f8, "f8e4m3f16")
run_benchmark(torch.sum, values_f8.half(), "f8e4m3f16_th") # torch.sum not support fp8
print("-" * 80)
